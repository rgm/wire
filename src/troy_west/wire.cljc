(ns troy-west.wire
  ...
  * Namespaced keywords enable composing and reusing subgraphs in larger flows
  ...
  There are functions for visualising graphs on clojure only; see
  troy-west.wire.graph.
  "
  (:require
   [clojure.set :as set]
...
(defn free-variables
  ...
  ([{:keys [dependents dependencies]} args]
   (set/difference (into #{} (keys dependents))
                   (set/union (into #{} (keys dependencies))
                              (into #{} (keys args))))))
